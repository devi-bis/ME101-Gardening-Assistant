...
void Terminate()												//Initiates program termination, returns robot to start location
{
	...
	while (SensorValue[S3] > 35){}				//Turns around until intensity sensor reading is low (into the black line)
	motor[motorA] = 0;
	motor[motorB] = 0;
...
bool Dig()
{
	...
	const float turnDeg = 500.0;					//Degrees one motor encoder needs to turn for a 90 degree turn - determined empirically
...
//	CRServo(-1);																	//Drill CCW
	...
	drillDist = (config + 1)*5 + clearance;			//Digs 3cm deep hole for Blue, 4cm for Green, 5cm for Yellow
